...
export default function InventoryResults({
  ...
  const [selectedItem,  SetLoanedItem] = useState<ProcessedItem | null>(null);
  ...
  const handleLoanSuccess = () => {
    alert("Loan created successfully!");
     SetLoanedItem(null);  // Close the loan popup
  };
  ...
  as well as filtering for duplicate items generated by the sql */
 ...
 items.forEach(item => {
  ...
  // Retrieve the raw earliest due date from the API (it could be undefined or null)
  let loanDueDate = item.earliest_due_date;

  // (No hard override happens here)
  ...
  if (existing) {
    ...
    if (loanDueDate) {
      // Use the earliest value between any duplicates
      if (!existing.earliest_due_date || new Date(loanDueDate) < new Date(existing.earliest_due_date)) {
        existing.earliest_due_date = loanDueDate;
      }
    }
  } else {
    processedItems.push({
      ...
      contributors: (contributor.first_name || contributor.last_name) ? [contributor] : []
    });
  }
});
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      {reservationItem && (
        <MakeReservationPopup
          ...
          earliestLoanDueDate={reservationItem.earliest_due_date ?? undefined} // Convert null to undefined
        />
      )}
      {infoItem && (
